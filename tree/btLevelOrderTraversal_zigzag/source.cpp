...
class Solution {
    public:
      vector<vector<int> > zigzagLevelOrder(TreeNode* root) {
        ...
        if( order.empty() ) return reverseOrder;
        bool reverseFlag = false;
        for(int i = 0; i< order.size(); i++){
            if(i % 2 == 1) reverseFlag = true;
            else reverseFlag = false;
            if( reverseFlag ){
                vector<int> newVec;
                for(int j = order[i].size() -1 ; j>= 0; j--)
                    newVec.push_back(order[i][j]);
                reverseOrder.push_back(newVec);
            }else
...
int main( int argc, char* argv[]){
    ...
    cout<<endl;
    vector<vector<int> > vec2= sl->zigzagLevelOrder(bsTree.root);
    print2DVec(vec2);
    //printVec(rv);
    // cout<<(bl == true? 1 :0);
    return 0;
