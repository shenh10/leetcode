...
class Solution {
    public:
     vector<string> binaryTreePaths(TreeNode* root) {
        ...
        vector<int> subVec;
        if( !root ) return vec;  
        findPath(root, subVec, vec);
        return vec;
        
    }
    void findPath(TreeNode* root, vector<int> subVec, vector<string> &vec){
            if( !root-> left && ! root->right){
                    subVec.push_back(root->val);
                    string s ;
                    for(int i = 0; i< subVec.size(); i++){
                        s += to_string(subVec[i]);
                        s += "->";
                    }
                    if(s.length() == 0) return;
                    else s= s.substr(0, s.length()-2);
                    cout<<s<<endl;
                    vec.push_back(s);
                    return;
            }
            subVec.push_back(root->val);
            if(root->left)
                findPath(root->left, subVec, vec);

            if(root->right)
                findPath(root->right, subVec, vec);
    }
