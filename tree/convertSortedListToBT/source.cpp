...
Solution:
better solution would be use a fast slow pointer.
*/
...
class Solution {
   ...
   TreeNode* root = NULL;
            ...
            iterateInsert(0, len, nums, root);
            return root;


        }
        void iterateInsert(int left, int right,vector<int> &nums, TreeNode* &root){
            if(left > right)
                return;
            if(left == right){
                insertValue(root, nums[left]);
                return;
            }
            
            int mid = left+ (right - left)/2;
            insertValue(root, nums[mid]);
            iterateInsert(left, mid, nums, root);
            if(mid + 1 >= right) return;
            iterateInsert(mid+1, right,nums, root);
