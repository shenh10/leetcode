...
Solution:
    The most important is : only when previous level linked then we can use recursive link next level, for that tree transversal is depth-first instead of brand-first.
*/
...
class Solution {
    public:
        ...
        void subConnect(TreeLinkNode* pre){
            if(pre -> left){
                if(pre->right){
                    pre->left->next = pre->right;
                }else{
TreeLinkNode* ptr = pre, *ptr_post = pre->left;
                while(ptr->next){
                    ptr = ptr->next;
                    if(ptr->left){
                        ptr_post->next = ptr->left;
                        ptr_post = ptr->left;
                    }
                    if(ptr->right){
                        ptr_post->next = ptr->right;
                        ptr_post = ptr->right;
                    }
            ...
            if(pre->right){
                TreeLinkNode* ptr = pre, *ptr_post = pre->right;
                while(ptr->next){
                    ptr = ptr->next;
                    if(ptr->left){
                        ptr_post->next = ptr->left;
             ...
             if(pre->left)   subConnect(pre->left);
             if(pre->right)   subConnect(pre->right);
        }
...
int main( int argc, char* argv[]){
    ...
    /*int arr[ ROW ][ COL ] = {{1,3,5,7},{10,11,16,20},{23,30,34,50}};
      ...
      print2DVec(vec);*/
    int arr[] = {2,1,3,0,7,9,1,2,INT_MIN,1,0,INT_MIN,INT_MIN,8,8,INT_MIN,INT_MIN,INT_MIN,INT_MIN,7,INT_MIN, INT_MIN, INT_MIN,INT_MIN,INT_MIN,INT_MIN,INT_MIN,INT_MIN,INT_MIN,INT_MIN,INT_MIN}; 
    //int arr[] = {1,2};
    vector<int> vec;
    vec.assign(arr, arr+31);
