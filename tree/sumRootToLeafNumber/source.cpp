...
class Solution {
   ...
   int sumNumbers(TreeNode* root) {
        ...
        if( !root ) return 0;  
        int sumup = 0;
        findPath(root, subVec, sumup);
        return sumup;
        
    }
    void findPath(TreeNode* root, vector<int> subVec, int& sumup){
            ...
            subVec.push_back(root->val);
            if(root->left)
                findPath(root->left, subVec, sumup);

            if(root->right)
                findPath(root->right, subVec, sumup);
    }
...
int main( int argc, char* argv[]){
    ...
    /*int arr[ ROW ][ COL ] = {{1,3,5,7},{10,11,16,20},{23,30,34,50}};
      ...
      print2DVec(vec);*/
    int arr[] = {1 , 2 ,5 ,INT_MIN, INT_MIN,5,9 };
    //int arr[] = {1,2};
    vector<int> vec;
